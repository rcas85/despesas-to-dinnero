...
/* Page transitions */
.page { 
  height:100dvh; height:100vh; display:flex; flex-direction:column; overflow:hidden;
...
function App() {
  ...
  // iOS Safari keyboard fix: when keyboard closes, viewport stays shifted
  useEffect(() => {
    const reset = () => {
      window.scrollTo(0, 0);
    ...
    // Also listen to visualViewport resize (keyboard open/close)
    const handleResize = () => {
...
function TripDetailPage({ trip, onBack, onCapture, onEdit, onExport, onSaveTrip, showToast }) {
  ...
  return (
    <div className="page">
      ...
      {showEditTrip && (
        <div className="modal-overlay" onClick={() => setShowEditTrip(false)} style={{ alignItems:"center" }}>
          <div className="modal-sheet" onClick={e => e.stopPropagation()} style={{ borderRadius:"var(--radius)", margin:"0 16px", maxHeight:"80vh" }}>
            <div className="modal-handle" />
